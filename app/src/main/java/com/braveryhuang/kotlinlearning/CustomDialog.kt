...
class CustomDialog(context: Context, val message: String, val permissions: List<String>) :
    ...
    override fun getNegativeButton(): View? {
        val view = findViewById<Button>(R.id.negativeBtn)
        return view
    }

    override fun getPositiveButton(): View {
        val view = findViewById<Button>(R.id.positiveBtn)
        return view
    }
    ...
    private fun buildPermissionsLayout() {
        for (permission in permissions) {
            ...
            if (permissionGroup != null && !groupSet.contains(permissionGroup)) {
                val permissionsLayout = findViewById<LinearLayout>(R.id.permissionsLayout)
